...
char RxBuffer[15];           //串口接收数组
u8 RxCount;                        //串口接收数据索引
u8 pre_cnt_rs;                  //数据接收进行标志
...
int main(void)
{
    ...
    uart2_init(115200);        
    printf("yuyuyuyuyuyuyuyuyranranranranranranranranranranran\n");    
   IWDG_Init();                                 //看门狗初始化
    GPIO_init();                                   //外部端口初始化
    SuperPort_RST();                      //网口复位
    TIM2_Int_Init(50-1,7200-1);//5ms计时
